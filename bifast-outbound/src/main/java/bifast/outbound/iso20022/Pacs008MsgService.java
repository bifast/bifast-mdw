...
public class Pacs008MsgService {
	...
	public FIToFICustomerCreditTransferV08 creditTransferRequest (Pacs008Seed seed) 
		...
		if (!(seed.getCrdtProxyIdValue().isBlank())) {
		...
		if ((!(seed.getCrdtType().isBlank())) ||
			(!(seed.getCrdtResidentStatus().isBlank())) ||
			(!(seed.getCrdtTownName().isBlank())) ||
			(!(seed.getDbtrType().isBlank())) ||
			(!(seed.getDbtrResidentStatus().isBlank())) ||
			(!(seed.getDbtrTownName().isBlank())) || 
			(!(seed.getEndToEndId().isBlank())) ) {
